...
export default function ApiKeyModal({ isOpen, onClose, onKeyUpdated }: ApiKeyModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-lg bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl p-6 sm:p-7 space-y-6 text-zinc-100">
        {/* Header */}
        <div className="flex items-center justify-between pb-4 border-b border-zinc-800/80">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg font-bold text-white tracking-tight">API Key Settings (BYOK)</h2>
              <p className="text-xs text-zinc-400">Bring Your Own Key for unlimited custom AI generations</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Security Notice */}
        <div className="p-3.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-start gap-3 text-xs text-emerald-300">
          <ShieldCheck className="w-4 h-4 shrink-0 mt-0.5 text-emerald-400" />
          <div className="leading-relaxed">
            <span className="font-semibold text-emerald-200">100% Client-Side Privacy:</span> Your keys are stored solely in your browser&apos;s <code className="bg-emerald-950/60 px-1 py-0.5 rounded font-mono">localStorage</code>. They are sent directly in request headers to Google/OpenAI and never logged or saved to any database.
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full bg-zinc-950 border border-zinc-800 rounded-xl px-3.5 py-2.5 text-xs text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-indigo-500 font-mono transition"
            />
        ...
        <div className="flex items-center justify-between pt-4 border-t border-zinc-800">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              onClick={handleSave}
              className="px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold shadow-lg shadow-indigo-600/25 transition flex items-center gap-1.5"
            >
